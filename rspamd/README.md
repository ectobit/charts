...
When `redis.enabled=true` or `externalRedis.enabled=true`, the chart renders `/etc/rspamd/local.d/history_redis.conf` with an upstream-compatible history key prefix by default:
...
Embedded Redis password authentication with `redis.auth.enabled=true` is not supported by this chart. Use `externalRedis.password` or `externalRedis.existingSecret` for authenticated Redis or Valkey.

## External Redis or Valkey

Set `externalRedis.enabled=true` to use a Redis-compatible service outside this chart, such as a separate Valkey release. Do not enable `redis.enabled` at the same time; the chart fails rendering when both modes are enabled.

When `externalRedis.existingSecret` is set, the chart-generated Secret stores Redis config templates without the password. An init container reads the password from the referenced Secret through `REDIS_PASSWORD`, writes final `redis.conf` and `history_redis.conf` files into an `emptyDir`, and the Rspamd container mounts those rendered files. Inline `externalRedis.password` is rendered into the chart-generated Secret, not a ConfigMap. TLS CA/client files are mounted only when `externalRedis.tls.enabled=true` and `externalRedis.tls.existingSecret` is set.
...
| config.historyRedis.enabled             | bool   | `true`                                                                                                                                                                                                                                                                                                                     | Render /etc/rspamd/local.d/history_redis.conf when embedded or external Redis is enabled.                                                                               |
...
| config.workerController.password        | string | `""`                                                                                                                                                                                                                                                                                                                       | Optional controller password. Rendered into a Secret, not a ConfigMap.                                                                                                  |
| config.workerController.secureIp        | list   | `["127.0.0.1","::1"]`                                                                                                                                                                                                                                                                                                      | IPs or CIDR ranges allowed to access the controller.                                                                                                                    |
| externalRedis.db                        | string | `""`                                                                                                                                                                                                                                                                                                                       | Optional Redis database number rendered as Rspamd `db`.                                                                                                                 |
| externalRedis.enabled                   | bool   | `false`                                                                                                                                                                                                                                                                                                                    | Use an external Redis-compatible backend, such as Valkey, instead of the embedded Redis dependency.                                                                     |
| externalRedis.existingSecret            | string | `""`                                                                                                                                                                                                                                                                                                                       | Existing Secret containing the Redis password. An init container reads it through REDIS_PASSWORD and writes the final Redis config to an emptyDir before Rspamd starts. |
| externalRedis.existingSecretPasswordKey | string | `"password"`                                                                                                                                                                                                                                                                                                               | Key in externalRedis.existingSecret containing the Redis password.                                                                                                      |
| externalRedis.host                      | string | `""`                                                                                                                                                                                                                                                                                                                       | External Redis or Valkey hostname.                                                                                                                                      |
| externalRedis.password                  | string | `""`                                                                                                                                                                                                                                                                                                                       | Optional Redis password. Rendered into a Secret, not a ConfigMap. Prefer existingSecret for production.                                                                 |
| externalRedis.port                      | int    | `6379`                                                                                                                                                                                                                                                                                                                     | External Redis or Valkey port.                                                                                                                                          |
| externalRedis.timeout                   | string | `""`                                                                                                                                                                                                                                                                                                                       | Optional Rspamd Redis timeout value.                                                                                                                                    |
| externalRedis.tls.caFilename            | string | `"ca.crt"`                                                                                                                                                                                                                                                                                                                 | CA filename inside externalRedis.tls.existingSecret.                                                                                                                    |
| externalRedis.tls.certFilename          | string | `""`                                                                                                                                                                                                                                                                                                                       | Optional client certificate filename inside externalRedis.tls.existingSecret.                                                                                           |
| externalRedis.tls.certKeyFilename       | string | `""`                                                                                                                                                                                                                                                                                                                       | Optional client certificate key filename inside externalRedis.tls.existingSecret.                                                                                       |
| externalRedis.tls.enabled               | bool   | `false`                                                                                                                                                                                                                                                                                                                    | Enable TLS for the external Redis connection.                                                                                                                           |
| externalRedis.tls.existingSecret        | string | `""`                                                                                                                                                                                                                                                                                                                       | Existing Secret containing the TLS CA and optional client certificate/key files.                                                                                        |
| externalRedis.tls.noVerify              | bool   | `false`                                                                                                                                                                                                                                                                                                                    | Disable TLS certificate verification. Leave false unless the backend certificate cannot be verified.                                                                    |
| externalRedis.tls.sni                   | string | `""`                                                                                                                                                                                                                                                                                                                       | Optional TLS SNI name.                                                                                                                                                  |
| externalRedis.username                  | string | `""`                                                                                                                                                                                                                                                                                                                       | Optional Redis ACL username.                                                                                                                                            |
...
| redis.auth.enabled                      | bool   | `false`                                                                                                                                                                                                                                                                                                                    | Embedded Redis password authentication is not supported by this chart. Use externalRedis.password or externalRedis.existingSecret for authenticated Redis or Valkey.    |
| redis.enabled                           | bool   | `false`                                                                                                                                                                                                                                                                                                                    | Enable the Redis dependency.                                                                                                                                            |
| replicaCount                            | int    | `1`                                                                                                                                                                                                                                                                                                                        |                                                                                                                                                                         |
| resources                               | object | `{}`                                                                                                                                                                                                                                                                                                                       |                                                                                                                                                                         |
